...
Date: 19.10.2014
Task: Merge sort
IDE: please, use Microsoft Visual Studio 2010
*/
...
void main() {
	...
	*/
	/*
		...
		1) read data from file "test_proekt_5.txt" (which should be located in the same directory with source files);
...
{
	string exit;
	clear_file("output_proekt_5.txt");//очистка файла перед использованием
	do 
	{
		//methods		
		load_or_write();
...
void load_arr()//загружаем массивы из файла
{
	int lenght, numofarr;			
	fstream myfile;			
	myfile.open("test_proekt_5.txt", ios::in);
...
void write_arr()//если вводим вручную
{
	int lenght, min, max;
...
{
	ofstream fOut;
	fOut.open("output_proekt_5.txt", ios::app);
